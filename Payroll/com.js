...
const COM_PRINT_PAYLOAD_KEY = "com_print_payload_v1";
const COM_PRINT_RETURN_KEY = "com_print_return_v1";
const COM_PRINT_PAGE_URL = "com-print.html";
const COM_CALCULATOR_URL = "com.html";
...
function parseFlexibleDate(value) {
  ...
  // 4) MM/DD/YYYY or M/D/YYYY or MM/DD/YY
...
function escapeHtml(str) {
  ...
  function buildPrintSheetHTML() {
    ...
    // OE for PRINT (Base + CC Fee + Total) without changing on-screen UI
    const tpNum = parseMoney(totalPrice);
    const baseOENum = Number.isFinite(baseOtherExpense) ? baseOtherExpense : parseMoney(oe);

    // Always compute the 3% amount for reference/verification
    const fee3pct = 0.03 * tpNum;

    // Only add fee into totals when addCreditFee=true
    const ccFeeNum = addCreditFee ? fee3pct : 0;
    const totalOENum = baseOENum + ccFeeNum;

    const baseOEText = formatMoneyUSD(baseOENum);

    // Default display: show $0.00 if not applying fee
    let ccFeeText = formatMoneyUSD(ccFeeNum);

    // Requirement: if user says fee is already included in Total Price,
    // show a verification message instead of $0.00 (only for Credit Card method)
...
document.addEventListener("DOMContentLoaded", function () {
  if (window.__com_gold_initialized) return;
  window.__com_gold_initialized = true;
  ...
  // Print (dedicated same-tab print page)
  ...
  if (printButton) {
    printButton.addEventListener("click", function (event) {
      ...
      sessionStorage.setItem(COM_PRINT_PAYLOAD_KEY, buildPrintSheetHTML());
      sessionStorage.setItem(COM_PRINT_RETURN_KEY, COM_CALCULATOR_URL);

      setTimeout(() => {
        window.location.href = COM_PRINT_PAGE_URL;
      }, 120);
    });
  }
}
...
function calculateCommission() {
  ...
  /*
    ...
    2) If tp === 539 → commission = $114.21 flat
    3) If tp > 539:
       - profitPercentage >= 25 → 21.19% of grossAmount
       - profitPercentage >= 20 → 20.00% of grossAmount
       - profitPercentage >= 15 → 17.50% of grossAmount
       - profitPercentage >= 10 → 15.00% of grossAmount
       - profitPercentage >= 5  → 10.00% of grossAmount
       - profitPercentage < 5   → 60 * pd
  */
  if (tp < 539) {
    baseCommission = 60;
  ...
  /* HARD FLOOR: technician commission can never be below $60 */
