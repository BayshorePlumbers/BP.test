...
document.addEventListener('DOMContentLoaded', () => {
  if (window.__htech_gold_initialized) return;
  window.__htech_gold_initialized = true;

  // =========================
  // Bayshore Standard Utilities (match payrollOPS behavior)
  // =========================
  const STATE_KEY = 'bs_htech_state_v2';
  const HTECH_PRINT_PAYLOAD_KEY = 'htech_print_payload_v1';
  const HTECH_PRINT_RETURN_KEY = 'htech_print_return_v1';
  const HTECH_PRINT_PAGE_URL = 'htech-print.html';
  const HTECH_CALCULATOR_URL = 'htech.html';
  ...
  // -------------------------
    ...
    // Prevent duplicate wiring (Gold Template behavior)
    if (el.dataset.bsSelectAllWired === "1") return;
    el.dataset.bsSelectAllWired = "1";
  ...
  function wireMoneyInput(input) {
    ...
    // Bayshore Standard: show raw numeric while editing
  ...
  const totalOEInput = document.getElementById('totalOE');
  ...
  const paymentMethodRow = document.getElementById('question2');
  ...
  const payMethodNotReceivedBtn = document.getElementById('payMethodNotReceived');

  const cashAmountRow = document.getElementById('questionCash');
  const cashAmountInput = document.getElementById('cashAmount');

  const checkNumberRow = document.getElementById('questionCheck');
  const checkNumberInput = document.getElementById('checkNumber');

  const cardFeeRow = document.getElementById('question3');
  const cardFeeAddedHidden = document.getElementById('cardFeeAdded');

  const canon_paymentReceived = document.getElementById('canon_paymentReceived');
  const canon_paymentMethod = document.getElementById('canon_paymentMethod');
  const canon_creditFeeAdded = document.getElementById('canon_creditFeeAdded');

  function writeCanon() {
    if (canon_paymentReceived) canon_paymentReceived.value = paymentReceivedHidden.value || '';
    if (canon_paymentMethod) canon_paymentMethod.value = paymentMethodHidden.value || '';
    if (canon_creditFeeAdded) canon_creditFeeAdded.value = cardFeeAddedHidden.value || '';
  }
  ...
  function syncCalculatorPrintSheet() {
    ...
    sheet.innerHTML = buildPrintSheetHTML({ skipRecalc: true });
    sheet.setAttribute('aria-hidden', 'false');
  }

  function readCanon() {
    if (canon_paymentReceived && canon_paymentReceived.value) paymentReceivedHidden.value = canon_paymentReceived.value;
    if (canon_paymentMethod && canon_paymentMethod.value) paymentMethodHidden.value = canon_paymentMethod.value;
    if (canon_creditFeeAdded && canon_creditFeeAdded.value) cardFeeAddedHidden.value = canon_creditFeeAdded.value;
  }
  ...
  function updatePaymentVisibility() {
    ...
    if (!received) {
      ...
      [payMethodCashBtn, payMethodCheckBtn, payMethodCardBtn, payMethodAccountsBtn, payMethodNotReceivedBtn].forEach(b => setQaPressed(b, false));
  ...
  function updatePaymentMethodFollowups() {
    ...
    // NOT RECEIVED => no followups + hard reset
  ...
  function computeCcFeeDisplay() {
    // Display-only per Master Standard: 3% of ENTERED TP when CARD and fee not already added
    const tp = readMoneyInput(totalPriceInput);
    const isCard = (paymentReceivedHidden.value === 'yes' && paymentMethodHidden.value === 'CARD');
    const feeAlreadyAdded = (cardFeeAddedHidden.value === 'yes');

    if (isCard && !feeAlreadyAdded) return tp * 0.03;
    return 0;
  }
  ...
  function calculateReport() {
    ...
    // Display-only OE breakdown (does NOT change existing profit logic)
    const ccFeeDisplay = computeCcFeeDisplay();
    if (ccFeeInput) ccFeeInput.value = formatMoney(ccFeeDisplay);
    if (totalOEInput) totalOEInput.value = formatMoney(otherExpenses + ccFeeDisplay);
    ...
    if (!techName || !techRate) {
      // Consistent hint styling (Gold Template)
  ...
  [
    ...
    [payMethodNotReceivedBtn, 'NOT_RECEIVED'],
  ].forEach(([btn, val]) => {
    btn.dataset.value = val;
    btn.addEventListener('click', () => {
      setChoiceButtons(
        [payMethodCashBtn, payMethodCheckBtn, payMethodCardBtn, payMethodAccountsBtn, payMethodNotReceivedBtn],
        paymentMethodHidden,
        val
      );
      updatePaymentMethodFollowups();
  ...
  function calculateDebounced() {
    ...
    calcTimer = setTimeout(calculateReport, 80);
  }

    document.querySelectorAll('input, textarea, select').forEach(input => {
    input.addEventListener('input', () => { saveStateDebounced(); calculateDebounced(); });
    input.addEventListener('change', () => { saveStateDebounced(); calculateDebounced(); });

    // Bayshore Standard: never wire select-all on hidden fields
    if (input.tagName === 'INPUT' && input.type === 'hidden') return;
    if (!input.hasAttribute('readonly') && !input.disabled) selectAllOnEdit(input);
  });
  ...
  function restoreState() {
    try {
      ...
      // Canonical hidden fields are authoritative for Q/A restore
      readCanon();
      ...
      [payMethodCashBtn, payMethodCheckBtn, payMethodCardBtn, payMethodAccountsBtn, payMethodNotReceivedBtn].forEach(b => {
        if (!b) return;
  ...
  // Initial formatting pass (so blanks become defaults)
    [totalPriceInput, materialExpensesInput, otherExpensesInput, cashAmountInput, ccFeeInput, totalOEInput].forEach(inp => {
    if (!inp) return;
  ...
  function buildPrintSheetHTML({ skipRecalc = false } = {}) {
    ...
    return `
      <div class="htech-print-root">
        <div class="print-header">
          <img src="BP.png" alt="Bayshore Plumbers Logo" class="logo" loading="eager">
          <h2>TECHNICIAN POTENTIAL KICKER DOCUMENT</h2>
        </div>
        ...
        <div class="print-body">
          <div class="no-break details-section">
            ...
            <table class="input-data">
              ...
              <tr><th>CC Fee (Auto):</th><td>${escapeHtml(getVal("ccFee"))}</td></tr>
              <tr><th>Total OE (Base + Fee):</th><td>${escapeHtml(getVal("totalOE"))}</td></tr>
              <tr><th>Project Duration:</th><td>${escapeHtml(getVal("pd"))}</td></tr>
              <tr><th>Job Description/Notes:</th><td>${escapeHtml(notesText)}</td></tr>
            </table>
          </div>
    ...
    sessionStorage.setItem(HTECH_PRINT_PAYLOAD_KEY, buildPrintSheetHTML());
    sessionStorage.setItem(HTECH_PRINT_RETURN_KEY, HTECH_CALCULATOR_URL);

    setTimeout(() => {
      window.location.href = HTECH_PRINT_PAGE_URL;
    }, 120);
  });
});